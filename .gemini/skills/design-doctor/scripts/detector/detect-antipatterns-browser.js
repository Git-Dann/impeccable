...
const REPEATED_KICKER_SKIP_SELECTOR = [
  ...
  '[class*="breadcrumb" i]',
  '[aria-hidden="true"]',
  '[data-design-doctor-allow-kickers]',
].join(',');
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__DESIGN_DOCTOR_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip design-doctor's own elements
    if (el.closest && el.closest('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner, .design-doctor-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.designDoctorExtension === 'true')
  ...
  styleEl.textContent = `
    @keyframes design-doctor-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .design-doctor-overlay:not(.design-doctor-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: design-doctor-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .design-doctor-overlay.design-doctor-visible {
      animation-play-state: running;
    }
    .design-doctor-overlay.design-doctor-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .design-doctor-overlay.design-doctor-hover .design-doctor-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .design-doctor-overlay.design-doctor-spotlight {
      z-index: 100002 !important;
    }
    .design-doctor-overlay.design-doctor-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .design-doctor-spotlight-backdrop {
      position: fixed;
    ...
    .design-doctor-spotlight-backdrop.design-doctor-visible {
      opacity: 1;
    }
    .design-doctor-hidden .design-doctor-overlay${EXTENSION_MODE ? '' : ':not(.design-doctor-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'design-doctor-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__DESIGN_DOCTOR_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('design-doctor-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('design-doctor-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('design-doctor-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._designDoctorOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('design-doctor-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._designDoctorOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._designDoctorOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('design-doctor-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    if (el._designDoctorOverlay) detachOverlay(el._designDoctorOverlay);
    const hasSlop = findings.some(f => RULE_CATEGORY[f.type || f.id] === 'slop');

    const fixed = isInFixedContext(el);
    const rect = el.getBoundingClientRect();
    const outline = document.createElement('div');
    outline.className = 'design-doctor-overlay';
    ...
    label.className = 'design-doctor-label';
    ...
    el._designDoctorOverlay = outline;
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('design-doctor-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('design-doctor-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'design-doctor-overlay design-doctor-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('design-doctor-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('design-doctor-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner, .design-doctor-tooltip')) continue;
      if (el.closest('[id^="design-doctor-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner, .design-doctor-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[design-doctor] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[design-doctor] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__DESIGN_DOCTOR_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__DESIGN_DOCTOR_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip design-doctor's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner, .design-doctor-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the design-doctor live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip design-doctor-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="design-doctor-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__DESIGN_DOCTOR_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__DESIGN_DOCTOR_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'design-doctor-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('design-doctor-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[design-doctor] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('design-doctor-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'design-doctor-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__DESIGN_DOCTOR_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('design-doctor-hidden');
        document.body.classList.toggle('design-doctor-hidden', visible);
        window.postMessage({ source: 'design-doctor-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('design-doctor-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('design-doctor-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('design-doctor-spotlight', isMatch);
              o.classList.toggle('design-doctor-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('design-doctor-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('design-doctor-spotlight');
          o.classList.remove('design-doctor-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'design-doctor-ready' }, '*');
  } else {
    if (window.__DESIGN_DOCTOR_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[design-doctor] scan failed', err);
        }
      };
  ...
  window.designDoctorDetect = detect;
  window.designDoctorDetectAsync = detectAsync;
  window.designDoctorScan = scan;
  window.designDoctorScanAsync = scanAsync;
  window.designDoctorCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.designDoctorAnalyzeVisualContrast = analyzeVisualContrast;
  window.designDoctorGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
