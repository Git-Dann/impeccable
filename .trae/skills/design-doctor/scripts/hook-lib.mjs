/**
 * Shared library for the Design Doctor design hook.
 *
...
import { DESIGN_DOCTOR_COMMAND } from './lib/provider.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export const ENVELOPE_PREFIX = '[design-doctor@1]';
...
const HOOK_IGNORE_MARKER_OPEN = '# design-doctor-hook-ignore-start';
const HOOK_IGNORE_MARKER_CLOSE = '# design-doctor-hook-ignore-end';
...
export function getConfigPath(cwd) {
  return path.join(cwd, '.design-doctor', 'config.json');
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, '.design-doctor', 'config.local.json');
}

export function getCachePath(cwd) {
  return path.join(cwd, '.design-doctor', 'hook.cache.json');
}

export function getPendingPath(cwd) {
  return path.join(cwd, '.design-doctor', 'hook.pending.json');
}
...
function looksLikeProjectRoot(dir) {
  return ['.git', 'package.json', '.design-doctor'].some((marker) => {
    try { return fs.existsSync(path.join(dir, marker)); } catch { return false; }
  });
}

// Where `.design-doctor/` (cache + config) lives for this event. Normally the
...
// .design-doctor), key to the edited file's nearest project root instead, so a
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run ${DESIGN_DOCTOR_COMMAND} audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see ${DESIGN_DOCTOR_COMMAND} audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see ${DESIGN_DOCTOR_COMMAND} audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) => [
    header,
    ...linesArr,
    ...(omitted ? [`... and more (see ${DESIGN_DOCTOR_COMMAND} audit).`] : []),
    '',
    footer,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = `... and more (see ${DESIGN_DOCTOR_COMMAND} audit).`;
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `${DESIGN_DOCTOR_COMMAND} hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.DESIGN_DOCTOR_HOOK_HARNESS;
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  let target = env?.DESIGN_DOCTOR_HOOK_LOG;
...
// silence-on-clean can set `DESIGN_DOCTOR_HOOK_QUIET=1` — runHook checks that
...
const STEER_LINE = 'That does not mean the design is good: keep following the project design system and the design-doctor skill guidance.';
...
export function appendDesignSystemNote(text, scanOptions) {
  ...
  return `${text}\n\n${ENVELOPE_PREFIX} DESIGN.md is newer than .design-doctor/design.json. Run ${DESIGN_DOCTOR_COMMAND} document to refresh the design-system sidecar.`;
}
...
function directiveFooter(display, opts = {}) {
  const ignoreFileCommand = `${DESIGN_DOCTOR_COMMAND} hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = opts.grouped
    ? `run \`${DESIGN_DOCTOR_COMMAND} hooks ignore-file <path>\` for the specific file`
  ...
  return [
    ...
    '',
    `Do not change intentional design just to satisfy the hook, and do not silence a real finding with an inline ignore comment to skip fixing it. Suppress a finding only after the user explicitly confirms it is intentional. Prefer a config ignore (one reviewable place, the commands below); reach for an inline \`design-doctor-disable <rule>\` comment only when the waiver must travel with a file that leaves the repo, such as an exported or standalone document. Prefer the narrowest persisted exception: run the exact \`${DESIGN_DOCTOR_COMMAND} hooks ignore-value ... --shared\` command shown next to a value-specific finding. For \`overused-font\`, use \`ignore-value\` for a specific font and use \`${DESIGN_DOCTOR_COMMAND} hooks ignore-rule overused-font --all-values\` only when the user asks to ignore overused fonts generally. For file-specific findings without an ignore-value command, ${fileIgnoreGuidance}; use \`${DESIGN_DOCTOR_COMMAND} hooks ignore-rule <id>\` only when the user asks to suppress the whole non-value-specific rule. Run ${DESIGN_DOCTOR_COMMAND} audit for the full pass.`,
  ].join('\n');
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.DESIGN_DOCTOR_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.DESIGN_DOCTOR_HOOK_DISABLED)) {
    ...
    // `.design-doctor/` (dedup and suppression need it), and an already-present
    // `.design-doctor/` dir marks a project that opted in. A non-UI edit, or a
    // clean UI edit in a project with no Design Doctor footprint, must be a
    // no-op on disk (issues #344, #305).
    if (freshGroups.length > 0
      || (cacheDirty && fs.existsSync(path.join(projectCwd, '.design-doctor')))) {
    ...
    if (truthy(env.DESIGN_DOCTOR_HOOK_QUIET) || config.quiet === true) {
