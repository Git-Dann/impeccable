  ...
  it('covers live chrome, replace, edit, annotations, insert, refresh recovery, accept/discard, and exit', async (t) => {
    ...
    try {
      ...
      const afterExit = await page.evaluate(() => ({
        ...
        hasGlobalBar: Boolean(window.__designDoctorLiveQuery?.('#design-doctor-live-global-bar')),
      }));
...
function loadConsolidatedFixture() {
  ...
  fixture.runtime = {
    ...
    stateProbe: {
      ...
      windowProperty: '__designDoctorStatefulMounts',
    ...
    steer: {
      ...
      expectSelector: 'h1.hero-title[data-design-doctor-steer="e2e"]',
      expectSourceContains: 'data-design-doctor-steer="e2e"',
    },
    probe: {
      expectLiveInit: true,
...
async function runReplaceDiscardRecoveryFlow({ page, tmp, evidence }) {
  ...
  assert.match(session.previewFile, /^node_modules\/\.design-doctor-live\/[^/]+\/manifest\.json$/);
...
async function runEditCopyFlow({ page, tmp, live, evidence }) {
  ...
  assert.doesNotMatch(sourceAfter, /contenteditable|data-design-doctor-editable|data-design-doctor-original-text|design-doctor-variants/, 'manual Apply does not leak live scaffolding');
}
...
async function runAcceptReplaceFlow({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-design-doctor-variants|design-doctor-variants-start|data-design-doctor-variant=/, 'accepted replace source is clean');
...
async function runInsertFlowWithRecovery({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-design-doctor-variants|design-doctor-variants-start|data-design-doctor-variant=/, 'accepted insert source is clean');
...
async function assertSvelteAdapterInjection(tmp) {
  ...
  assert.doesNotMatch(appHtml, /live\.js|design-doctor-live-start/, 'Svelte adapter leaves src/app.html untouched');
  assert.match(layout, /DesignDoctorLiveRoot|design-doctor-live-svelte-start/, 'Svelte adapter patches layout with live root');
}

async function assertShadowChrome(page) {
  const result = await page.evaluate(() => {
    const host = document.getElementById('design-doctor-live-root');
    const root = host?.shadowRoot;
    return {
      hasHost: Boolean(host),
      hasShadowRoot: Boolean(root),
      hasGlobalBar: Boolean(root?.getElementById('design-doctor-live-global-bar')),
      documentHasGlobalBar: Boolean(document.querySelector('#design-doctor-live-global-bar')),
    };
  });
...
async function assertHostileCssIsolation(page) {
  ...
  const hostileStyle = await page.addStyleTag({
    content: `
      button, div, input, svg, #design-doctor-live-root, design-doctor-live-root * {
...
async function waitForGlobalBarStable(page) {
  ...
  await page.waitForFunction(
    () => {
      const bar = window.__designDoctorLiveQuery?.('#design-doctor-live-global-bar');
      if (!bar) return false;
...
async function assertSelectedElementChrome(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    const bar = q('#design-doctor-live-bar');
    const picker = q('#design-doctor-live-picker');
    const badge = q('#design-doctor-live-edit-badge');
    const highlight = q('#design-doctor-live-highlight');
    const input = q('#design-doctor-live-input');
    return {
      hasBar: Boolean(bar),
...
async function selectAction(page, label) {
  ...
  const opened = await page.evaluate(() => {
    const bar = window.__designDoctorLiveQuery('#design-doctor-live-bar');
  ...
  await page.waitForFunction(
    (expected) => {
      const picker = window.__designDoctorLiveQuery('#design-doctor-live-picker');
...
async function clickEditBadgeAction(page, label) {
  const clicked = await page.evaluate((expected) => {
    const badge = window.__designDoctorLiveQuery('#design-doctor-live-edit-badge');
...
async function clickPendingTrash(page) {
  ...
  const clicked = await page.evaluate(() => {
    const dock = window.__designDoctorLiveQuery('#design-doctor-live-pending-dock');
...
async function waitForSvelteAcceptComplete(tmp, session, { timeout }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      ...
      && !/data-design-doctor-variants|design-doctor-variants-start|data-design-doctor-variant=/.test(source)
    ) {
      return;
    }
...
function assertCompletedRealSource(tmp, id) {
  ...
  assert.match(snapshot.previewFile || '', /node_modules\/\.design-doctor-live\/[^/]+\/manifest\.json/);
}
...
function readSessionSnapshot(tmp, id) {
  const file = join(tmp, '.design-doctor/live/sessions', `${id}.snapshot.json`);
...
async function assertNoCyclingZero(page) {
  const text = await page.evaluate(() => window.__designDoctorLiveQuery('#design-doctor-live-bar')?.textContent || '');
...
async function readStateProbe(page) {
  return page.evaluate(() => ({
    ...
    mounts: window.__designDoctorStatefulMounts,
  }));
}
...
function assertFinalSourceClean(tmp) {
  const source = readFileSync(join(tmp, ROUTE_FILE), 'utf-8');
  assert.doesNotMatch(source, /node_modules\/\.design-doctor-live|data-design-doctor-variants|design-doctor-variants-start|data-design-doctor-variant=/);
}
...
function readJournalEvents(tmp) {
  const dir = join(tmp, '.design-doctor/live/sessions');
...
function createEvidenceWriter({ page, tmp, live, consoleErrors }) {
  ...
  return {
    async capture(name) {
      ...
      try {
        const dom = await page.evaluate(() => {
          ...
          return {
            ...
            liveState: window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.debugState?.() || null,
            localSession: localStorage.getItem('design-doctor-live-session'),
          };
        });
      ...
      copyDirIfExists(join(tmp, '.design-doctor/live/sessions'), join(dir, 'sessions'));
...
function listPreviewFiles(tmp) {
  const root = join(tmp, 'node_modules/.design-doctor-live');
...
function createArtifactRoot() {
  const explicit = process.env.DESIGN_DOCTOR_SVELTE_DEEPSEEK_ARTIFACT_DIR;
