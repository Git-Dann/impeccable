 ...
 *   2. Symlinks the real .claude/skills/design-doctor into the workspace so
...
function loadSkillBody() {
  ...
  // and the scripts the skill references live at .claude/skills/design-doctor/
  ...
  md = md
    ...
    .replaceAll('{{scripts_path}}', '.claude/skills/design-doctor/scripts')
 ...
 *
 * - `.claude/skills/design-doctor` is symlinked at the SOURCE skill dir (not
 *   the built `.claude/skills/design-doctor/`) so the test exercises whatever
...
export function prepareWorkspace({ files = {}, skillVersion = null } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'design-doctor-skill-test-'));
  const skillDest = path.join(dir, '.claude', 'skills', 'design-doctor');
...
export function makeTools(workspace, extraEnv = {}) {
  ...
  const tools = {
    bash: tool({
      description:
        'Run a bash command in the workspace root. Use this to invoke skill scripts (e.g. `node .claude/skills/design-doctor/scripts/load-context.mjs`).',
