...
function stageFixture(name) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'design-doctor-fixture-'));
  ...
  writeFileSync(join(tmp, '.design-doctor', 'live', 'config.json'), JSON.stringify(fixture.config));
    ...
    it('live-inject --port adds the script tag to every config file', () => {
      ...
      try {
        ...
        const ignored = execFileSync('git', [
          ...
          '.design-doctor/live/sessions/example.jsonl',
          '.design-doctor/live/previews/example/v1.html',
          '.design-doctor/live/deferred-svelte-component-accepts.json',
          'src/lib/design-doctor/DesignDoctorLiveRoot.svelte',
          'src/lib/design-doctor/__runtime.js',
          'src/lib/design-doctor/a4ac4e74/v3.svelte',
        ], { cwd: tmp, encoding: 'utf-8' });
        assert.match(ignored, /\.design-doctor\/live\/server\.json/);
        assert.match(ignored, /\.design-doctor\/live\/sessions\/example\.jsonl/);
        assert.match(ignored, /\.design-doctor\/live\/previews\/example\/v1\.html/);
        assert.match(ignored, /\.design-doctor\/live\/deferred-svelte-component-accepts\.json/);
        assert.match(ignored, /src\/lib\/design-doctor\/DesignDoctorLiveRoot\.svelte/);
        assert.match(ignored, /src\/lib\/design-doctor\/__runtime\.js/);
        assert.match(ignored, /src\/lib\/design-doctor\/a4ac4e74\/v3\.svelte/);
        if (result.adapter === 'sveltekit') {
          ...
          const root = readFileSync(join(tmp, 'src/lib/design-doctor/DesignDoctorLiveRoot.svelte'), 'utf-8');
          assert.match(layout, /design-doctor-live-svelte-start/, 'SvelteKit layout got the adapter marker');
          assert.match(layout, /DesignDoctorLiveRoot/, 'SvelteKit layout renders the adapter host');
          assert.doesNotMatch(appHtml, /design-doctor-live-start/, 'SvelteKit app.html must remain untouched');
        ...
        for (const r of result.results) {
          ...
          assert.match(body, /design-doctor-live-start/);
    ...
    it('live-inject --remove strips the script tag cleanly', () => {
      ...
      try {
        ...
        if (result.adapter === 'sveltekit') {
          const layout = readFileSync(join(tmp, 'src/routes/+layout.svelte'), 'utf-8');
          const appHtml = readFileSync(join(tmp, 'src/app.html'), 'utf-8');
          assert.doesNotMatch(layout, /DesignDoctorLiveRoot/);
          assert.doesNotMatch(layout, /design-doctor-live-svelte-start/);
          assert.doesNotMatch(appHtml, /design-doctor-live-start/);
          assert.equal(existsSync(join(tmp, 'src/lib/design-doctor/DesignDoctorLiveRoot.svelte')), false);
          return;
        }
        for (const r of result.results) {
          const body = readFileSync(join(tmp, r.file), 'utf-8');
          assert.doesNotMatch(body, /design-doctor-live-start/);
