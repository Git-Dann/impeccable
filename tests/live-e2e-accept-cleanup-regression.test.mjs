 ...
 * exposes the accepted element under stale data-design-doctor runtime wrappers.
 *
 * Run with:
...
describe('live-e2e accept cleanup regression', () => {
  it('cleans accepted LLM DOM wrappers after carbonize cleanup', async (t) => {
    ...
    try {
      ...
      assert.doesNotMatch(finalSource, /data-design-doctor-variants="/, 'source variants wrapper removed');
      assert.doesNotMatch(finalSource, /design-doctor-variants-start/, 'source variants markers removed');
      assert.doesNotMatch(finalSource, /design-doctor-carbonize-start/, 'source carbonize markers removed');
      assert.doesNotMatch(finalSource, /data-design-doctor-variant="/, 'source variant wrapper removed');
...
async function collectAcceptedDomDiagnostic(page, selector) {
  return page.evaluate((sel) => {
    const matches = [...document.querySelectorAll(sel)];
    const clean = matches.filter((el) => !el.closest('[data-design-doctor-variants],[data-design-doctor-variant],[data-design-doctor-carbonize]'));
    const stale = matches.filter((el) => el.closest('[data-design-doctor-variants],[data-design-doctor-variant],[data-design-doctor-carbonize]'));
    const local = {};
    for (const key of [
      'design-doctor-live-session',
      'design-doctor-live-session-handled',
      'design-doctor-live-session-scroll',
    ]) {
    ...
    return {
      ...
      remainingVariantsWrapperCount: document.querySelectorAll('[data-design-doctor-variants]').length,
      remainingVariantWrapperCount: document.querySelectorAll('[data-design-doctor-variant]').length,
      liveBarText: document.querySelector('#design-doctor-live-bar')?.textContent || '',
      localStorage: local,
      heroHtml: matches.map((el) => el.outerHTML.slice(0, 500)),
      staleAncestorHtml: stale.map((el) => {
        const ancestor = el.closest('[data-design-doctor-variants],[data-design-doctor-variant]');
...
function isSourceClean(source) {
  return !source.includes('data-design-doctor-variants=')
    && !source.includes('design-doctor-variants-start')
    && !source.includes('design-doctor-carbonize-start')
    && !source.includes('data-design-doctor-variant=');
}
