...
} from '../../cli/lib/design-doctor-config.mjs';

describe('cli/lib/design-doctor-config', () => {
  let root;
  ...
  test('writing consent gitignores config.local.json via .git/info/exclude', () => {
    ...
    expect(exclude).toContain('.design-doctor/config.local.json');
    ...
    expect((again.match(/design-doctor-config-ignore-start/g) || []).length).toBe(1);
  ...
  test('writeDetectionConfig local ignores do not create an implicit design-system override', () => {
    ...
    expect(readFileSync(join(root, '.git', 'info', 'exclude'), 'utf-8')).toContain('.design-doctor/config.local.json');
  });
