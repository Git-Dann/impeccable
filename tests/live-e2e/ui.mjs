 ...
 *
 * Selector strategy: live-browser.js uses deterministic ids (`design-doctor-live-*`)
...
const BAR_ID = '#design-doctor-live-bar';
const GLOBAL_BAR_ID = '#design-doctor-live-global-bar';
const PICKER_ID = '#design-doctor-live-picker';
const EDIT_BADGE_ID = '#design-doctor-live-edit-badge';
const PENDING_DOCK_ID = '#design-doctor-live-pending-dock';
...
const STEER_CHAT_ID = '#design-doctor-live-page-chat';
const STEER_INPUT_ID = '#design-doctor-live-page-chat-input';
const PICK_TOGGLE = '#design-doctor-live-pick-toggle';
...
const INSERT_TOGGLE = '#design-doctor-live-insert-toggle';
const DETECT_TOGGLE = '#design-doctor-live-detect-toggle';
const DETECT_BADGE = '#design-doctor-live-detect-badge';
const DESIGN_TOGGLE = '#design-doctor-live-design-toggle';
const DESIGN_HOST = '#design-doctor-live-design-host';
const EXIT_BUTTON = '#design-doctor-live-exit';
const INSERT_INPUT_ID = '#design-doctor-live-insert-input';
const INSERT_CREATE_ID = '#design-doctor-live-insert-create';
const ANNOTATION_ID = '#design-doctor-live-annot';
const ANNOTATION_PINS_ID = '#design-doctor-live-annot-pins';
const ANNOTATION_CLEAR_ID = '#design-doctor-live-annot-clear';

/**
 * Wait for the live handshake to complete:
 *   - window.__DESIGN_DOCTOR_LIVE_INIT__ set
 *   - global bar mounted
...
export async function waitForHandshake(page, { timeout = 20_000 } = {}) {
  ...
  await page.waitForFunction(
    (sel) => Boolean(window.__designDoctorLiveQuery?.(sel)),
    GLOBAL_BAR_ID,
    { timeout },
  );
...
export async function assertBottomBarIdle(page, { timeout = 5_000 } = {}) {
  await installLiveQueryHelpers(page);
  await page.waitForFunction(
    ({ ids }) => ids.every((sel) => Boolean(window.__designDoctorLiveQuery(sel))),
    {
      ids: [
        GLOBAL_BAR_ID,
        PICK_TOGGLE,
  ...
  const snapshot = await page.evaluate(({ pickSel, insertSel, detectSel, designSel }) => {
    const q = window.__designDoctorLiveQuery;
    return {
...
function installLiveQueryHelpersInPage() {
  window.__designDoctorLiveQuery = (selector) => {
    const root = window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.root?.()
      || window.__DESIGN_DOCTOR_LIVE_UI_ROOT__
      || null;
    return root?.querySelector?.(selector) || document.querySelector(selector);
  };
  window.__designDoctorLiveQueryAll = (selector) => {
    const root = window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.root?.()
      || window.__DESIGN_DOCTOR_LIVE_UI_ROOT__
      || null;
...
async function clickLiveControl(page, selector) {
  ...
  const clicked = await page.evaluate((sel) => {
    const el = window.__designDoctorLiveQuery(sel);
...
async function readControlActive(page, selector) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => window.__designDoctorLiveQuery(sel)?.dataset.active === 'true', selector);
}
...
async function ensureLiveControlActive(page, selector, active) {
  ...
  await page.waitForFunction(
    ({ sel, expected }) => window.__designDoctorLiveQuery(sel)?.dataset.active === (expected ? 'true' : 'false'),
...
export async function runPickInsertToggleSmoke(page) {
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__designDoctorLiveQuery(pickSel)?.dataset.active === 'true'
      && window.__designDoctorLiveQuery(insertSel)?.dataset.active === 'false',
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__designDoctorLiveQuery(pickSel)?.dataset.active === 'false'
      && window.__designDoctorLiveQuery(insertSel)?.dataset.active === 'true',
    { pickSel: PICK_TOGGLE, insertSel: INSERT_TOGGLE },
    { timeout: 5_000 },
  );
...
export async function runDetectSmoke(page, { expectMinCount = 1 } = {}) {
  ...
  await page.waitForFunction(
    ({ badgeSel, expectMin }) => {
      const badge = window.__designDoctorLiveQuery(badgeSel);
      const count = parseInt(badge?.textContent || '0', 10);
      const overlays = document.querySelectorAll('.design-doctor-overlay').length;
  ...
  await ensureLiveControlActive(page, PICK_TOGGLE, true);
  await page.waitForFunction(
    () => [...document.querySelectorAll('.design-doctor-overlay')]
  ...
  await page.waitForFunction(
    ({ badgeSel }) => {
      const badge = window.__designDoctorLiveQuery(badgeSel);
      return document.querySelectorAll('.design-doctor-overlay').length === 0
...
export async function runDesignPanelSmoke(page, { title = '', rawText = '' } = {}) {
  ...
  await page.waitForFunction(
    ({ hostSel, titleText }) => {
      const host = window.__designDoctorLiveQuery(hostSel);
  ...
  await page.evaluate((hostSel) => {
    const root = window.__designDoctorLiveQuery(hostSel)?.shadowRoot;
    const raw = [...(root?.querySelectorAll('.tab') || [])].find((btn) => /Raw/i.test(btn.textContent || ''));
    raw?.click();
  }, DESIGN_HOST);
  await page.waitForFunction(
    ({ hostSel, expected }) => {
      const text = window.__designDoctorLiveQuery(hostSel)?.shadowRoot?.textContent || '';
  ...
  await page.waitForFunction(
    ({ hostSel, toggleSel }) => {
      const panel = window.__designDoctorLiveQuery(hostSel)?.shadowRoot?.querySelector('.panel');
      const toggle = window.__designDoctorLiveQuery(toggleSel);
...
export async function clickExitLiveMode(page) {
  ...
  await page.waitForFunction(
    ({ barSel }) => {
      const bar = window.__designDoctorLiveQuery?.(barSel);
      return window.__DESIGN_DOCTOR_LIVE_INIT__ === false && (!bar || !bar.isConnected);
    },
...
export async function drawAnnotationPinAndStroke(page, {
  ...
  await page.waitForFunction(
    (pinsSel) => Boolean(window.__designDoctorLiveQuery(pinsSel)?.querySelector('input')),
    ANNOTATION_PINS_ID,
    { timeout: 5_000 },
  );
  await page.evaluate(({ pinsSel, value }) => {
    const input = window.__designDoctorLiveQuery(pinsSel)?.querySelector('input');
  ...
  await page.waitForFunction(
    ({ pinsSel, expected }) => {
      const pins = window.__designDoctorLiveQuery(pinsSel);
  ...
  await page.waitForFunction(
    ({ annotSel, clearSel }) => {
      const annot = window.__designDoctorLiveQuery(annotSel);
      const clear = window.__designDoctorLiveQuery(clearSel);
...
async function waitForAnnotationRect(page) {
  ...
  return page.evaluate((sel) => {
    const rect = window.__designDoctorLiveQuery(sel).getBoundingClientRect();
    return {
      left: rect.left,
      top: rect.top,
      width: rect.width,
...
async function hideAnnotationOverlay(page) {
  await page.evaluate(() => {
    const annot = window.__designDoctorLiveQuery('#design-doctor-live-annot');
...
async function ensurePickerActive(page) {
  ...
  const clicked = await page.evaluate((sel) => {
    const btn = window.__designDoctorLiveQuery(sel);
    if (!btn) return false;
    btn.click();
    return true;
  }, PICK_TOGGLE_ID);
  ...
  await page.waitForFunction(
    (sel) => window.__designDoctorLiveQuery(sel)?.dataset.active === 'true',
    PICK_TOGGLE_ID,
    { timeout: 5_000 },
  );
}
...
export async function waitForCycling(page, expectedCount, { timeout = 30_000 } = {}) {
  ...
  try {
    ...
    if (process.env.DESIGN_DOCTOR_E2E_DEBUG) {
      const snapshot = await page.evaluate((barSel) => {
        const query = window.__designDoctorLiveQuery || ((sel) => document.querySelector(sel));
        const root = window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.root?.() || window.__DESIGN_DOCTOR_LIVE_UI_ROOT__ || null;
        const bar = query(barSel);
        const toast = query('#design-doctor-live-toast');
        const wrapper = document.querySelector('[data-design-doctor-variants]');
        return {
          liveInit: window.__DESIGN_DOCTOR_LIVE_INIT__,
          adapter: window.__DESIGN_DOCTOR_LIVE_ADAPTER__,
          ...
          wrapper: wrapper ? { preview: wrapper.dataset.designDoctorPreview, count: wrapper.dataset.designDoctorVariantCount, html: wrapper.outerHTML.slice(0, 600) } : null,
          debugState: window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.debugState?.() || null,
          storage: localStorage.getItem('design-doctor-live-session'),
          scripts: document.querySelectorAll('script[data-design-doctor-live-script]').length,
  ...
  try {
    ...
    return await withTimeout(
      page.evaluate((barSel) => {
        const wrapper = window.__designDoctorLiveQuery('[data-design-doctor-variants]');
        if (wrapper) {
          ...
          const idx = visible ? parseInt(visible.dataset.designDoctorVariant || '0', 10) : 0;
          if (idx > 0) return idx;
        }
        const bar = window.__designDoctorLiveQuery(barSel);
        if (!bar) return null;
...
export async function clickEditCopy(page) {
  ...
  await page.waitForFunction(
    () => window.__designDoctorLiveQuery('[data-design-doctor-editable="true"]')?.isContentEditable === true,
    { timeout: 5_000 },
  );
}
...
export async function clickSaveEdit(page) {
  ...
  await page.waitForFunction(
    () => !window.__designDoctorLiveQuery('[data-design-doctor-editable="true"]'),
    { timeout: 5_000 },
  );
}
...
async function clickEditBadgeButton(page, label) {
  const proxyRect = await page.evaluate((text) => {
    const proxies = [...document.querySelectorAll('[data-design-doctor-edit-badge-proxy="true"]')];
...
export async function assertApplyDockVisible(page, expectedCount, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ dockSel, expected }) => {
      const dock = window.__designDoctorLiveQuery(dockSel);
...
export async function preparePageForBarInteraction(page) {
  await page.evaluate(() => {
    for (const el of window.__designDoctorLiveQueryAll('astro-dev-toolbar')) {
...
export async function waitForSteerInputFocused(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (inputSel) => {
      const input = window.__designDoctorLiveQuery(inputSel);
      const active = window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__?.activeElementDeep?.()
...
export async function waitForSteerInputValue(page, value, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ inputSel, value: expected }) => window.__designDoctorLiveQuery(inputSel)?.value === expected,
...
export async function waitForSteerLocked(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (sel) => window.__designDoctorLiveQuery(sel)?.dataset.processing === 'true',
    STEER_CHAT_ID,
    { timeout },
  );
}
...
export async function waitForSteerUnlocked(page, { timeout = 15_000 } = {}) {
  await page.waitForFunction(
    (sel) => {
      const chat = window.__designDoctorLiveQuery(sel);
      const input = window.__designDoctorLiveQuery('#design-doctor-live-page-chat-input');
...
async function ensureToggleActive(page, selector, shouldBeActive) {
  ...
  await page.locator(selector).click({ timeout: 5_000 });
  await page.waitForFunction(
    ({ sel, active }) => window.__designDoctorLiveQuery(sel)?.dataset.active === (active ? 'true' : 'false'),
...
export async function runInsertFlow(page, {
  ...
  await page.waitForFunction(() => {
    const line = window.__designDoctorLiveQuery('#design-doctor-live-insert-line');
  ...
  await page.waitForFunction(
    ({ sel, value }) => window.__designDoctorLiveQuery(sel)?.value === value,
