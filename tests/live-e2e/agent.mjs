 ...
 *    `<style data-design-doctor-css="ID">` block with `@scope ([data-design-doctor-variant="N"])`
 *    rules, a `data-design-doctor-params` JSON manifest covering range + steps + toggle
...
export const STEER_MARKER_ATTR = 'data-design-doctor-steer';
 ...
 * @property {string}        scopedCss   Contents of the <style data-design-doctor-css>
 ...
 *   - <style data-design-doctor-css="ID"> with @scope rules per variant
 *   - data-design-doctor-params manifest with range + steps + toggle kinds
  ...
  return {
    ...
    async generateVariants(event, context = {}) {
      ...
      const scopedCss = useAstroGlobalCss
        ? [
            `[data-design-doctor-variant="1"] > ${tag} {`,
            '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
            '}',
            `[data-design-doctor-variant="2"] > ${tag} { font-weight: 900; }`,
            `[data-design-doctor-variant="2"][data-p-face="serif"] > ${tag} { font-family: ui-serif, serif; }`,
            `[data-design-doctor-variant="2"][data-p-face="mono"]  > ${tag} { font-family: ui-monospace, monospace; }`,
            `[data-design-doctor-variant="3"] > ${tag} { text-transform: uppercase; letter-spacing: 0.04em; }`,
            `[data-design-doctor-variant="3"][data-p-italic] > ${tag} { font-style: italic; }`,
          ].join('\n')
        : [
            ...
            '}',
            '@scope ([data-design-doctor-variant="3"]) {',
...
function generateInsertFakeVariants(context = {}) {
  ...
  const scopedCss = useAstroGlobalCss
    ? [
        '[data-design-doctor-variant="1"] .inserted-copy {',
        '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
        '}',
        '[data-design-doctor-variant="2"] .inserted-copy { font-weight: 900; }',
        '[data-design-doctor-variant="2"][data-p-face="serif"] .inserted-copy { font-family: ui-serif, serif; }',
        '[data-design-doctor-variant="2"][data-p-face="mono"]  .inserted-copy { font-family: ui-monospace, monospace; }',
        '[data-design-doctor-variant="3"] .inserted-copy { text-transform: uppercase; letter-spacing: 0.04em; }',
        '[data-design-doctor-variant="3"][data-p-italic] .inserted-copy { font-style: italic; }',
      ].join('\n')
    : [
        '@scope ([data-design-doctor-variant="1"]) {',
        '  :scope .inserted-copy {',
...
export const HOIST_ATTR = 'data-design-doctor-hoist-id';
...
function normalizeVariantSelectorQuotes(css) {
  return String(css).replace(
    /\[data-design-doctor-variant=(['"])(\d+)\1\]/g,
    (_match, _quote, id) => `[data-design-doctor-variant="${id}"]`,
  );
}
...
function hasBaseVariantRule(scopedCss, variantId, styleMode) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return new RegExp(String.raw`\[data-design-doctor-variant=${q}\](?:\s|>|\.|#|\[${HOIST_ATTR}=)`).test(scopedCss);
  }
  return new RegExp(String.raw`@scope\s*\(\s*\[data-design-doctor-variant=${q}\]\s*\)`).test(scopedCss);
}

function renderBaseVariantRule(variantId, styleMode) {
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-design-doctor-variant="${variantId}"] > * {`,
      '  --design-doctor-variant-ready: 1;',
      '}',
    ].join('\n');
  }
  return [
    `@scope ([data-design-doctor-variant="${variantId}"]) {`,
    '  :scope > * { --design-doctor-variant-ready: 1; }',
    '}',
  ].join('\n');
}
...
// `style="..."`, and tag the element with `data-design-doctor-hoist-id="N"`.
...
function renderHoistedInlineStyleRule({ variantId, hoistId, declarations, styleMode }) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-design-doctor-variant="${variantId}"] ${target} {`,
 ...
 *   - data-design-doctor-params stays a single-quoted JSON string (JSX-legal)
 */
function renderVariantsBlock({ sessionId, indent, output, commentSyntax, file, styleMode }) {
  ...
  const styleLines = isJsx
    ? [
        indent + '  <style data-design-doctor-css="' + sessionId + '">{`',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  `}</style>',
      ]
    : [
        indent + '  <style' + (isAstroGlobalCss ? ' is:inline' : '') + ' data-design-doctor-css="' + sessionId + '">',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  </style>',
      ];
  ...
  const variantBlocks = output.variants.map((v, i) => {
    ...
    const paramsAttr = v.params && v.params.length
      ? " data-design-doctor-params='" + attrEscape(JSON.stringify(v.params), { svelte: isSvelte }) + "'"
      : '';
    let styleAttr = '';
    ...
    return [
      ...
      indent + '  <div data-design-doctor-variant="' + idx + '"' + styleAttr + paramsAttr + '>',
      indent + '    ' + inner,
      indent + '  </div>',
    ].join('\n');
  });
...
async function writeSvelteComponentVariants({ tmp, wrapInfo, event, output }) {
  ...
  for (let i = 0; i < output.variants.length; i++) {
    ...
    if (isInsert && /\bdata-design-doctor-[\w-]*\s*=/.test(markup)) {
      throw new Error(`Svelte insert variant ${variantId} contains preview-only data-design-doctor attributes`);
    }
...
function svelteCssForVariant(scopedCss, variantId, tag) {
  ...
  const rewritten = chunks
    .join('\n')
    .replace(new RegExp(String.raw`\\[data-design-doctor-variant=["']${variantId}["']\\]\\s*>\\s*`, 'g'), '')
    .replace(new RegExp(String.raw`\\[data-design-doctor-variant=["']${variantId}["']\\][^{]*>\\s*`, 'g'), '')
    .replace(/:scope(?:\[[^\]]+\])?\s*>\s*/g, '')
    .replace(/:scope(?:\[[^\]]+\])?/g, tag)
    .split('\n')
...
function extractVariantCssChunks(css, variantId) {
  ...
  for (const line of lines) {
    if (line.includes(`[data-design-doctor-variant="${variantId}"]`) || line.includes(`[data-design-doctor-variant='${variantId}']`)) {
      collecting = true;
      depth = 0;
 ...
 * @param {string} opts.scriptsDir Path to the design-doctor scripts dir.
 ...
 *
 *   1. Locate the carbonize block (bracketed by `design-doctor-carbonize-start`
 *      and `design-doctor-carbonize-end`).
 ...
 *      `<div data-design-doctor-variant="N" style="display: contents"|...>` wrapper
...
async function runCarbonizeCleanup({ tmp, file, sessionId /* , variant */ }) {
  ...
  const startRe = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*design-doctor-carbonize-start\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n');
  const endRe   = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*design-doctor-carbonize-end\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n?');
  ...
  // 2. Unwrap the temporary `<div data-design-doctor-variant="N" ...>` placed
  // around the accepted content. For JSX targets, live-accept also adds an
  // outer `<div data-design-doctor-carbonize>` so the carbonize block and accepted
  // node occupy one child slot; strip that shell after the accepted node is
  // clean.
  body = unwrapDivAttributeWrapper(body, 'data-design-doctor-variant', { expandSingleLineContainer: true });
  body = unwrapDivAttributeWrapper(body, 'data-design-doctor-carbonize');

  // 3. Strip any `data-design-doctor-hoist-id` attributes the normalize step
  ...
  body = body.replace(/\s+data-design-doctor-hoist-id="[^"]*"/g, '');
