...
export async function runPreActions(page, actions) {
  ...
  const wasActive = await page.evaluate((sel) =>
    window.__designDoctorLiveQuery?.(sel)?.dataset.active === 'true',
...
async function liveUiSnapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasShadowRoot: Boolean(document.getElementById('design-doctor-live-root')?.shadowRoot),
      ...
      wrapper: wrapper ? { preview: wrapper.dataset.designDoctorPreview, count: wrapper.dataset.designDoctorVariantCount, html: wrapper.outerHTML.slice(0, 800) } : null,
