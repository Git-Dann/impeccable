...
// to iterate on. Set DESIGN_DOCTOR_E2E_ONLY=<name>[,<name>...] to scope the run.
const onlyNames = parseFixtureFilter(process.env.DESIGN_DOCTOR_E2E_ONLY);
...
if (missingOnlyNames.length > 0) {
  throw new Error(`Unknown DESIGN_DOCTOR_E2E_ONLY fixture(s): ${missingOnlyNames.join(', ')}`);
}

const manualOnly = process.env.DESIGN_DOCTOR_E2E_MANUAL_ONLY === '1'
  || process.env.DESIGN_DOCTOR_E2E_MANUAL_ONLY === 'true';
const reloadVariants = process.env.DESIGN_DOCTOR_E2E_RELOAD_VARIANTS === '1'
  || process.env.DESIGN_DOCTOR_E2E_RELOAD_VARIANTS === 'true';
const scenarioNames = parseFixtureFilter(process.env.DESIGN_DOCTOR_E2E_SCENARIOS);
const liveE2eTestTimeoutMs = readPositiveIntEnv('DESIGN_DOCTOR_E2E_TEST_TIMEOUT_MS');
...
for (const { name, fixture } of fixtures) {
  describe(`live-e2e · ${name} (${fixture.runtime.styling || 'unknown-styling'})`, () => {
    it('drives the full click → Go → cycle → accept cycle', liveE2eTestOptions, async (t) => {
      ...
      if (manualOnly || process.env.DESIGN_DOCTOR_E2E_MANUAL_SCENARIO) {
      ...
      // Pick the agent. `DESIGN_DOCTOR_E2E_AGENT=llm` opts into Claude first,
      ...
      const agentMode = process.env.DESIGN_DOCTOR_E2E_AGENT || 'fake';
      let agent;
      if (agentMode === 'llm') {
        ...
        if (!agent) {
          t.skip(`DESIGN_DOCTOR_E2E_AGENT=llm with provider=${llmConfig.provider} requires ${llmConfig.requiredEnv}`);
          return;
        }
      ...
      const variantContentSelector = isInsert
        ? (usesSvelteComponentPreview ? '.inserted-copy' : '[data-design-doctor-variant="2"] .inserted-copy')
        : usesSvelteComponentPreview
        ? pickSelector
        : '[data-design-doctor-variant="2"] > :first-child';
      ...
      try {
        ...
        if (isInsert) {
          ...
          if (process.env.DESIGN_DOCTOR_E2E_DEBUG) {
            const barText = await page.evaluate(() => {
              const bar = document.querySelector('#design-doctor-live-bar');
        ...
        if (svelteComponentSession) {
          ...
          assert.doesNotMatch(routeBody, /data-design-doctor-variants="/, 'Svelte route source is not edited during generation');
        } else {
          assert.match(after, /data-design-doctor-variants="/, 'wrapper inserted');
        }
        if (isInsert) {
          if (svelteComponentSession) {
            assert.equal(svelteComponentSession.manifest.mode, 'insert', 'Svelte insert uses component preview mode');
          } else {
            assert.match(after, /data-design-doctor-mode="insert"/, 'insert mode wrapper');
            assert.doesNotMatch(after, /data-design-doctor-variant="original"/, 'insert has no original variant');
          }
        ...
        if (svelteComponentSession) {
          ...
          assert.match(after, /<style is:inline data-design-doctor-css="/, 'Astro live CSS uses an inline compiler-bypassing style block');
          assert.match(
            after,
            /\[data-design-doctor-variant="1"\]\s*>\s*(?:h1|\.[\w-]+)/,
          ...
          assert.doesNotMatch(after, /@scope \(\[data-design-doctor-variant="1"\]\)/, 'Astro live CSS does not use raw @scope');
        } else {
          assert.match(after, /<style data-design-doctor-css="/, 'colocated <style> block present');
          assert.match(after, /@scope \(\[data-design-doctor-variant="1"\]\)/, 'scoped CSS for variant 1');
          assert.match(after, /@scope \(\[data-design-doctor-variant="2"\]\)/, 'scoped CSS for variant 2');
          assert.match(after, /@scope \(\[data-design-doctor-variant="3"\]\)/, 'scoped CSS for variant 3');
        }
        ...
        if (agentMode === 'fake') {
          const paramsSource = svelteComponentSession
            ...
            : after;
          assert.match(paramsSource, svelteComponentSession ? /"1"\s*:/ : /data-design-doctor-params=/, 'params manifest emitted');
        ...
        for (const targetVariant of cycleSequence) {
          ...
          if (agentMode === 'fake' && targetVariant === 2 && !checkedVariantTwoStyle) {
            await page.waitForFunction((sel) => {
              const query = window.__designDoctorLiveQuery || ((s) => document.querySelector(s));
            ...
            if (variantWeight !== '900') {
              const styleSnapshot = await evaluatePageWithTimeout(
                ...
                (sel) => {
                  const query = window.__designDoctorLiveQuery || ((s) => document.querySelector(s));
                  const el = query(sel) || document.querySelector(sel);
                  const styleEl = document.querySelector('style[data-design-doctor-css]');
                  const rules = [];
        ...
        if (svelteComponentTarget) {
          ...
          const snapshotPath = join(tmp, '.design-doctor/live/sessions', `${svelteComponentTarget.manifest.id}.snapshot.json`);
          const snapshot = JSON.parse(readFileSync(snapshotPath, 'utf-8'));
          assert.equal(snapshot.phase, 'completed');
          assert.equal(snapshot.sourceFile, svelteComponentTarget.manifest.sourceFile);
          assert.doesNotMatch(snapshot.sourceFile, /node_modules\/\.design-doctor-live/);
        }
        assert.doesNotMatch(final, /data-design-doctor-variants="/,    'variants wrapper removed');
        assert.doesNotMatch(final, /design-doctor-variants-start/,      'variants-start marker removed');
        assert.doesNotMatch(final, /design-doctor-carbonize-start/,     'carbonize-start marker removed');
        assert.doesNotMatch(final, /design-doctor-carbonize-end/,       'carbonize-end marker removed');
        assert.doesNotMatch(final, /data-design-doctor-carbonize=/,     'carbonize wrapper removed');
        assert.doesNotMatch(final, /data-design-doctor-variant="/,      'no leftover variant scaffolding');
        if (isInsert) {
          if (agentMode === 'fake') {
    ...
    if (shouldRunScenario('manual') && Array.isArray(fixture.runtime.manualEditScenarios) && fixture.runtime.manualEditScenarios.length > 0) {
      const manualScenarioFilter = process.env.DESIGN_DOCTOR_E2E_MANUAL_SCENARIO || '';
    ...
    if (shouldRunScenario('annotations') && fixture.runtime.liveChrome?.annotations) {
      it('uploads annotations with generate and still accepts the variant', liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          assert.match(generateEvent.screenshotPath, /\.design-doctor\/live\/annotations\//, 'annotation screenshot is stored under live annotations');
...
async function captureLiveE2eFailure({ name, fixture, session, sourceFile, error, log = () => {} }) {
  const root = process.env.DESIGN_DOCTOR_E2E_ARTIFACT_DIR;
  ...
  try {
    ...
    copyDirIfExists(join(tmp, '.design-doctor', 'live'), join(dir, 'design-doctor-live'));
    copyDirIfExists(join(tmp, 'node_modules', '.design-doctor-live'), join(dir, 'design-doctor-live-preview'));
...
async function createManualScenarioAgent(t, scenario = {}) {
  const requested = (process.env.DESIGN_DOCTOR_E2E_MANUAL_AGENT || process.env.DESIGN_DOCTOR_E2E_AGENT || 'auto')
    .trim()
    .toLowerCase();
...
function maybeWrapMalformedAckProbe(agent, scenario, probeState, t) {
  ...
  return {
    ...
    async applyManualEdits(event, context = {}) {
      ...
      const buffer = JSON.parse(readFileSync(join(context.tmp, '.design-doctor/live/pending-manual-edits.json'), 'utf-8'));
...
async function waitForAcceptedSelectionReady(page, selector, { timeout }) {
  await page.waitForFunction(
    (sel) => {
      ...
      for (const el of all) {
        if (el.closest('[data-design-doctor-variants],[data-design-doctor-variant]')) return false;
      }
      return true;
    },
    selector,
...
async function readLiveSessionStorage(page) {
  return page.evaluate(() => {
    const raw = localStorage.getItem('design-doctor-live-session');
  ...
  try {
    await page.waitForFunction(
      ({ variant, count }) => {
        ...
        const bar = query('#design-doctor-live-bar');
    ...
    const snapshot = await page.evaluate(() => {
      ...
      return {
        ...
        wrapper: wrapper ? { preview: wrapper.dataset.designDoctorPreview, count: wrapper.dataset.designDoctorVariantCount, html: wrapper.outerHTML.slice(0, 500) } : null,
      };
...
async function waitForAcceptedDom(page, selector, { allowVariantRoot = false, timeout = 20_000 } = {}) {
  await page.waitForFunction(
    ({ sel, allowVariantRoot }) => {
      ...
      if (all.length < 1) return false;
      for (const el of all) {
        if (el.closest('[data-design-doctor-variants]')) return false;
        if (el.closest('[data-design-doctor-carbonize]')) return false;
        if (!allowVariantRoot && el.closest('[data-design-doctor-variant]')) return false;
      }
      return true;
    },
...
async function waitForSourceClean(filePath, timeoutMs, { svelteComponentTarget: knownSvelteTarget = null } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const dirty =
      ...
      last.includes('data-design-doctor-variants=') ||
      last.includes('design-doctor-variants-start') ||
      last.includes('design-doctor-carbonize-start') ||
      last.includes('data-design-doctor-carbonize=') ||
      last.includes('data-design-doctor-variant=');
...
function sourceShadowTargetFor(filePath) {
  ...
  if (!body.includes('data-design-doctor-preview="source-shadow"')) return null;
  const match = body.match(/\bdata-design-doctor-source-file=(["'])(.*?)\1/);
  if (!match) return null;
  const root = filePath.includes('/.design-doctor/')
    ? filePath.slice(0, filePath.indexOf('/.design-doctor/'))
    : dirname(filePath);
...
function svelteComponentTargetFor(filePath) {
  ...
  const markers = [
    `${sep}node_modules${sep}.design-doctor-live${sep}`,
    `${sep}src${sep}lib${sep}design-doctor${sep}`,
  ];
...
async function locateSessionFile(tmp) {
  ...
  for (const f of candidates) {
    ...
    if (
      body.includes('data-design-doctor-variants=') ||
      body.includes('design-doctor-carbonize-start') ||
      body.includes('design-doctor-variants-start')
    ) {
      return f;
    }
  }
...
function walkSvelteComponentManifests(root) {
  const results = [];
  const stack = [
    join(root, 'node_modules/.design-doctor-live'),
    join(root, 'src/lib/design-doctor'),
  ];
  while (stack.length) {
    const dir = stack.pop();
    let entries;
