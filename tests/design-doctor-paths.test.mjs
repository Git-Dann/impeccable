/**
 * Tests for project-local Design Doctor path resolution.
 * Run with: node --test tests/design-doctor-paths.test.mjs
 */
...
describe('design-doctor project paths', () => {
  let tmp;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'design-doctor-paths-'));
  });
  ...
  it('resolves the generated design sidecar under .design-doctor', () => {
    assert.equal(getDesignSidecarPath(tmp), join(tmp, '.design-doctor', 'design.json'));
  ...
  it('uses .design-doctor/live/config.json as the default live config path', () => {
  ...
  it('falls back to legacy scripts/config.json when no new live config exists', () => {
    const scriptsDir = join(tmp, 'skills', 'design-doctor', 'scripts');
  ...
  it('lets DESIGN_DOCTOR_LIVE_CONFIG override both new and legacy config locations', () => {
    const override = join(tmp, 'custom-live-config.json');
    mkdirSync(join(tmp, '.design-doctor', 'live'), { recursive: true });
    writeFileSync(getLiveConfigPath(tmp), '{"source":"new"}');
    writeFileSync(override, '{"source":"override"}');

    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: { DESIGN_DOCTOR_LIVE_CONFIG: 'custom-live-config.json' } }),
      override,
    );
  });

  it('places live server, session, and annotation state under .design-doctor/live', () => {
    assert.equal(getLiveServerPath(tmp), join(tmp, '.design-doctor', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp), join(tmp, '.design-doctor', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp), join(tmp, '.design-doctor', 'live', 'annotations'));
  });

  it('places .design-doctor state under the active monorepo child project', () => {
    ...
    assert.equal(getDesignSidecarPath(tmp, options), join(projectRoot, '.design-doctor', 'design.json'));
    assert.equal(getLiveConfigPath(tmp, options), join(projectRoot, '.design-doctor', 'live', 'config.json'));
    assert.equal(getLiveServerPath(tmp, options), join(projectRoot, '.design-doctor', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp, options), join(projectRoot, '.design-doctor', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp, options), join(projectRoot, '.design-doctor', 'live', 'annotations'));
    assert.equal(getCritiqueDir(tmp, options), join(projectRoot, '.design-doctor', 'critique'));
    assert.equal(getLegacyLiveServerPath(tmp, options), join(projectRoot, '.design-doctor-live.json'));
  ...
  it('does not let a root live config shadow a child project live config path', () => {
    ...
    mkdirSync(join(tmp, '.design-doctor', 'live'), { recursive: true });
    writeFileSync(join(tmp, '.design-doctor', 'live', 'config.json'), '{"source":"root"}');

    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: {}, targetPath: 'apps/admin/src/App.jsx' }),
      join(tmp, 'apps', 'admin', '.design-doctor', 'live', 'config.json'),
    );
  });
