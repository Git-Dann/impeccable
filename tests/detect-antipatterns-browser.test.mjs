...
import { filterDetectionFindings } from '../cli/lib/design-doctor-config.mjs';
...
describe('detectUrl — browser-only fixtures', () => {
  ...
  it('typography side-by-side: element-level flag cases get regular overlays', async () => {
    ...
    try {
      ...
      await page.evaluate(() => { window.__DESIGN_DOCTOR_CONFIG__ = { autoScan: false }; });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.designDoctorScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector('.design-doctor-banner')),
          overlays: document.querySelectorAll('.design-doctor-overlay:not(.design-doctor-banner)').length,
        };
      });
  ...
  it('browser API: visual contrast fallback resolves readable image backgrounds without overlays', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner').length;
        const analyses = await window.designDoctorAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
        const after = document.querySelectorAll('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner').length;
        return {
          before,
          after,
  ...
  it('browser API: visual contrast scan decorates visible findings without scrolling by default', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.designDoctorScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.designDoctorDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.designDoctorScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll('.design-doctor-overlay:not(.design-doctor-banner)').length,
          labels: document.querySelectorAll('.design-doctor-label').length,
          analyses: window.designDoctorGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          scrollEvents,
          maxScrollY,
          finalScrollY: window.scrollY,
          syncScanIsArray: Array.isArray(syncScanResult),
          syncDetectIsArray: Array.isArray(syncDetectResult),
          hasAsyncApi: typeof window.designDoctorScanAsync === 'function' && typeof window.designDoctorDetectAsync === 'function',
        };
      });
      ...
      assert.equal(result.syncScanIsArray, true, `designDoctorScan should keep a synchronous Array return: ${JSON.stringify(result)}`);
      assert.equal(result.syncDetectIsArray, true, `designDoctorDetect should keep a synchronous Array return: ${JSON.stringify(result)}`);
      ...
      const refreshedOverlayResult = await page.evaluate(async () => {
        ...
        const initialGroups = window.designDoctorScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        while (
          Date.now() < deadline &&
          !/low contrast/i.test(target?._designDoctorOverlay?.textContent || '')
        ) {
          const nextButton = target?._designDoctorOverlay?.querySelector('button:last-of-type');
          if (nextButton) nextButton.click();
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._designDoctorOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._designDoctorOverlay?.textContent || '',
          labelVariants,
          overlayConnected: Boolean(target?._designDoctorOverlay?.isConnected),
        };
      });
      ...
      const lazyResult = await page.evaluate(async () => {
        ...
        return {
          ...
          labels: document.querySelectorAll('.design-doctor-label').length,
          analyses: window.designDoctorGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          targetHasOverlay: Boolean(target?._designDoctorOverlay),
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise(resolve => setTimeout(resolve, 50));
        await window.designDoctorScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._designDoctorOverlay;
        target?.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'instant' });
        await new Promise(resolve => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._designDoctorOverlay),
          targetOverlayConnected: Boolean(target?._designDoctorOverlay?.isConnected),
          overlays: document.querySelectorAll('.design-doctor-overlay:not(.design-doctor-banner)').length,
          analyses: window.designDoctorGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
        };
      });
  ...
  it('extension mode remove cancels pending lazy visual contrast work', async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        window.__designDoctorMessages = [];
        window.addEventListener('message', event => {
          if (event.source !== window || !event.data?.source?.startsWith('design-doctor-')) return;
          window.__designDoctorMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window.designDoctorGetLastVisualContrastAnalyses()
            .some(item => item.status === 'unresolved' && item.reason === 'text outside viewport')
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window.designDoctorGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__designDoctorMessages
          .filter(message => message.source === 'design-doctor-results').length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll('.design-doctor-overlay').length,
          targetHasOverlay: Boolean(target?._designDoctorOverlay),
          resultsAfterRemove,
        };
      });
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__designDoctorMessages.some(message => message.source === 'design-doctor-error')
          ) {
            await new Promise(resolve => setTimeout(resolve, 25));
          }
          return {
            ready: window.__designDoctorMessages.some(message => message.source === 'design-doctor-ready'),
            results: window.__designDoctorMessages.some(message => message.source === 'design-doctor-results'),
            errors: window.__designDoctorMessages
              .filter(message => message.source === 'design-doctor-error')
  ...
  it('extension mode echoes scan ids on result messages', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__designDoctorMessages.some(message =>
            message.source === 'design-doctor-results' &&
            message.scanId === 'scan-2'
          )
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const resultMessage = window.__designDoctorMessages.find(message =>
          message.source === 'design-doctor-results' &&
          message.scanId === 'scan-2'
        );
        return {
  ...
  it('browser API: designDoctorDetect is pure, designDoctorScan decorates', async () => {
    const puppeteer = await import('puppeteer');
    const browser = await puppeteer.default.launch({
      headless: true,
      args: process.env.CI ? ['--no-sandbox', '--disable-setuid-sandbox'] : [],
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        const before = document.querySelectorAll('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner').length;
        const findings = window.designDoctorDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner').length;
      ...
      const decorated = await page.evaluate(() => {
        const groups = window.designDoctorScan();
        const overlays = document.querySelectorAll('.design-doctor-overlay, .design-doctor-label, .design-doctor-banner').length;
  ...
  it('browser API: async scan and detect reject instead of throwing synchronously', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.designDoctorScanAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          const detect = await window.designDoctorDetectAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          return { scan, detect };
