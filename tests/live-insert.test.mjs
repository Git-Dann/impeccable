...
describe('buildInsertWrapperLines', () => {
  it('produces an insert scaffold without an original variant', () => {
    ...
    assert.match(joined, /design-doctor-variants-start abc12345/);
    assert.match(joined, /data-design-doctor-variants="abc12345"/);
    assert.match(joined, /data-design-doctor-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-design-doctor-variant="original"/);
  });
  ...
  it('keeps marker comments inside the wrapper for JSX', () => {
    ...
    const joined = lines.join('\n');
    assert.match(joined, /^\s*<div data-design-doctor-variants="jsx12345"/m);
    assert.match(joined, /{\/\* design-doctor-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-design-doctor-variant="original"/);
  });
});
...
describe('live-insert CLI integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'design-doctor-insert-test-'));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-design-doctor-mode="insert"/);
    assert.match(after, /design-doctor-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf('design-doctor-variants-start ins12345'));
    assert.ok(after.indexOf('design-doctor-variants-start ins12345') < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-design-doctor-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf('design-doctor-variants-start ins99999') < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf('design-doctor-variants-start ins99999'));
  });
  ...
  return (
    ...
    assert.match(after, /data-design-doctor-mode="insert"/);
    assert.match(after, /{\/\* design-doctor-variants-start jsxins01 \*\/}/);
  });
