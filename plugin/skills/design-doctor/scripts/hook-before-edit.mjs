#!/usr/bin/env node
/**
 * Design Doctor design hook — Cursor preToolUse write gate.
 *
...
async function detectProposedHtml(detector, content, filePath, scanOptions) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'design-doctor-pre-'));
...
function cursorBlockMessage(findings, filePath, config, cwd) {
  ...
  const blocked = rendered.replace(
    '[design-doctor@1] Design hook findings requiring review',
    '[design-doctor@1] Design Doctor design hook blocked this write before it landed. Design hook findings requiring review',
  );
...
async function main() {
  if (truthy(process.env.DESIGN_DOCTOR_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so Design Doctor is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow({
      ...audit,
      findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.DESIGN_DOCTOR_HOOK_DEBUG) {
    process.stderr.write(`[design-doctor-hook-before-edit] ${err}\n`);
  }
