/**
 * CLI-side reader/writer for the unified `.design-doctor` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.design-doctor', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.design-doctor', 'config.local.json');
}
...
/**
 * Detector filters shared by `npx design-doctor detect` and the design hook.
...
const EXCLUDE_OPEN = '# design-doctor-config-ignore-start';
const EXCLUDE_CLOSE = '# design-doctor-config-ignore-end';
const EXCLUDE_PATTERNS = ['.design-doctor/config.local.json'];
