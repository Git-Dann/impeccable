...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/design-doctor/DesignDoctorLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- design-doctor-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- design-doctor-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import DesignDoctorLiveRoot from '$lib/design-doctor/DesignDoctorLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'design-doctor-live-root';

  onMount(() => {
    let host = document.querySelector('design-doctor-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('design-doctor-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.designDoctorLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-design-doctor-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.designDoctorLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__DESIGN_DOCTOR_LIVE_ADAPTER__ = 'sveltekit';
    window.__DESIGN_DOCTOR_LIVE_UI_ROOT__ = root;
    window.__DESIGN_DOCTOR_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.designDoctorLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__DESIGN_DOCTOR_LIVE_UI_ROOT__ === root) delete window.__DESIGN_DOCTOR_LIVE_UI_ROOT__;
      if (window.__DESIGN_DOCTOR_LIVE_CHROME_MOUNT__?.root === root) delete window.__DESIGN_DOCTOR_LIVE_CHROME_MOUNT__;
      if (window.__DESIGN_DOCTOR_LIVE_ADAPTER__ === 'sveltekit') delete window.__DESIGN_DOCTOR_LIVE_ADAPTER__;
    };
  });
</script>
`;
