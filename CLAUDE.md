...
There is **one** user-invocable skill, `design-doctor`, with **23 commands** underneath it. Users type `/design-doctor polish`, `/design-doctor audit`, etc. The skill is defined in `skill/`:
...
- `scripts/pin.mjs` — creates/removes lightweight redirect shims so users can have `/audit` as a standalone shortcut that delegates to `/design-doctor audit`.
...
PRODUCT.md at the project root carries a `## Register` section with a bare value (`brand` or `product`). `/design-doctor init` asks about register first because it shapes every downstream answer.
...
Editorial brief is at `docs/STYLE.md`. Read it before editing READMEs or other user-facing copy. The rules there exist to keep AI-tell prose from creeping in.
...
The build system compiles the design-doctor skill from `skill/` to provider-specific formats in `dist/`. The default build is source-first and does not sync tracked root harness folders; the release build performs the tracked distribution sync:

```bash
bun run build            # Build dist/ output without syncing root harness dirs
bun run build:release    # Build dist/ output and sync root harness dirs + plugin/
...
- `{{available_commands}}` — auto-populated list of commands (from `DESIGN_DOCTOR_SUB_COMMANDS` in `scripts/lib/utils.js`)
...
DESIGN_DOCTOR_E2E_ONLY=vite8-react-modal bun run test:live-e2e # scope to one fixture
DESIGN_DOCTOR_E2E_DEBUG=1 bun run test:live-e2e                # dump page DOM + dev-server tail on failure
```
...
**LLM agent (opt-in)**: set `DESIGN_DOCTOR_E2E_AGENT=llm` to swap the fake agent for `tests/live-e2e/agents/llm-agent.mjs`, which calls Claude (default Haiku 4.5) via `@anthropic-ai/sdk`. Requires `ANTHROPIC_API_KEY` in env; the test runner skips with a clear message when it's unset. Override the model with `DESIGN_DOCTOR_E2E_LLM_MODEL=claude-sonnet-4-6` if Haiku produces unreliable JSON. Caching is on — live.md is the cacheable prefix, and after the first call subsequent fixtures pay only the cache-read rate. Pass rate on a typical sweep is 18/19; the modal fixture's intrinsic state-loss flake is amplified by LLM latency and may need a re-run. **This path hits the API and costs money** — keep it out of CI unless you really want it there.
...
DESIGN_DOCTOR_SKILL_BEHAVIOR_MODELS=gemini-3.1-flash-lite bun run test:skill-behavior   # scope to one provider
DESIGN_DOCTOR_SKILL_BEHAVIOR_VERBOSE=1 bun run test:skill-behavior          # dump per-scenario trace JSON to stderr (use when iterating)
```
...
**Auth** lives in repo-root `.env` (copied from `~/code/design-doctor-evals/.env`, gitignored). Providers skip cleanly when their key is unset; they don't fail.
...
6. `/design-doctor polish` → loads `reference/polish.md`
7. `/design-doctor audit` → loads `reference/audit.md`
8. existing SvelteKit project → agent reads at least one project code file
9. `context.mjs` emits `UPDATE_AVAILABLE` (seeded newer version) → agent surfaces it but does **not** auto-run `npx design-doctor skills update`
10. scoped command with no PRODUCT.md → proceeds without forcing init
11. `/design-doctor shape` with no PRODUCT.md → diverts into `reference/init.md`
12. natural-language build intent with no PRODUCT.md → diverts into `reference/init.md`
13. `/design-doctor teach` → diverts into `reference/init.md` (alias)
14. PRODUCT.md with `## Platform: ios` → `context.mjs` emits the native NEXT STEP and the agent loads `reference/ios.md`
15. same iOS fixture, `/design-doctor audit` → agent loads `reference/audit.native.md` (route-instead variant)
...
The CLI lives in this repo under `cli/`: `cli/bin/` (entry + sub-commands), `cli/engine/` (the detect-antipatterns rule engine + browser variant), `cli/lib/` (shared helpers). Published to npm as `@gitwork/design-doctor` (CLI binary: `design-doctor`).

```bash
npx design-doctor detect [file-or-dir-or-url...]   # detect anti-patterns
npx design-doctor detect --fast --json src/         # regex-only, JSON output
npx design-doctor live                              # start browser overlay server
npx design-doctor skills install                    # install skills
npx design-doctor --help                            # show help
```
...
- After bumping, run `bun run build:release` so the committed `./plugin` subtree (`plugin/.claude-plugin/plugin.json` + `plugin/skills/design-doctor/SKILL.md`) is regenerated to the new version. The build validator (`validatePluginVersions` in `scripts/build.js`) fails if `marketplace.json`, the `./plugin` manifest, or the bundled `SKILL.md` frontmatter disagree with `plugin.json` — this guards the marketplace install path against version drift (issue #274).
...
2. Add a changelog entry in the format the release script expects (see `scripts/release.mjs`). Skill entries use a bare `vX.Y.Z` label; CLI and extension entries use the prefixed forms `CLI vX.Y.Z` and `Extension vX.Y.Z`. The release script extracts notes by matching this label, so the prefix matters.
...
If you need to fix release notes after the fact (typo, missing thank-you, formatting bug): `gh release edit <tag> --notes-file <md>`.

## Adding New Commands

All commands live under `/design-doctor`. To add a new one:
...
4. Add the command name to `DESIGN_DOCTOR_SUB_COMMANDS` in `scripts/lib/utils.js`
...
`cli/engine/detect-antipatterns.mjs` is the source of truth for the rule engine. It powers the CLI and the Chrome extension. Three places stay in sync:
...
6. **Verify against a live page**: run `npx design-doctor detect tests/fixtures/antipatterns/{rule-id}.html` (CLI/jsdom path) and load the fixture in a browser with the detector overlay active (browser path) — the two adapter paths can disagree, so manual checks catch what the fixture test can't.
...
The eval framework lives in a separate private repo at `~/code/design-doctor-evals/`. It measures whether the `/design-doctor` skill improves or harms AI-generated frontend design by running the same brief through a model with and without the skill loaded.
...
```bash
cd ~/code/design-doctor-evals
bun run serve            # dashboard on http://localhost:8723
```

The eval runners read this repo's skill from `../design-doctor/skill/` and staged provider skills from `../design-doctor/build/_data/dist/*` (the local clone directory name may still be `design-doctor`; adjust the relative path if you've renamed it). Run `bun run build` in this repo before an eval sweep if you want the Claude/Gemini staged skills to reflect your latest edits.
