...
Use this packet for the initial public submission of Design Doctor. The release build creates the upload at `dist/openai-plugin.zip`.

## Submission type

Choose **Skills only**. Design Doctor has no MCP server. The package includes one skill, its referenced scripts and files, a branded icon, and a `PostToolUse` lifecycle hook.

The hook runs locally after supported file edits, checks changed UI files for known anti-patterns, and returns findings to the active coding-agent session. It does not call a Design Doctor service. Codex asks each user to review and trust the hook before it can run.

## Listing

- Plugin name: Design Doctor
- Publisher: Gitwork
- Short description: Design and refine interfaces
- Long description: Create, critique, and refine frontend interfaces with your coding agent. Design Doctor provides 23 focused design commands, live browser iteration for exploring visual directions, and automatic checks that flag common design anti-patterns as you work.
- Category: Creativity
- Website: https://github.com/git-dann/impeccable
- Support: https://github.com/git-dann/impeccable/issues
- Terms: https://github.com/git-dann/impeccable/blob/main/LICENSE
- Logo: `dist/openai/design-doctor/assets/icon.png`

Use the verified Gitwork business identity. The generated manifest uses the same publisher name in both `author.name` and `interface.developerName`.

## Starter prompts

1. critique this interface and prioritize what to fix.
2. craft a distinctive landing page, then polish the result.
3. start design-doctor live so I can explore bolder or more delightful variants.
...
- Expected behavior: Invoke Design Doctor, load the audit and brand references, inspect the existing HTML, CSS, PRODUCT.md, and DESIGN.md, then run the bundled detector where applicable.
...
- Fixture data: The public `demos/landing-demo/` folder in `git-dann/impeccable`.

### 2. Final polish pass

- User prompt: `polish demos/landing-demo/index.html and fix the rough edges without changing the brand.`
- Expected behavior: Invoke Design Doctor, load the polish and brand references, preserve the existing design tokens, inspect the page in a browser when available, and make scoped edits.
...
- Expected behavior: Do not invoke Design Doctor. Explain that the plugin is scoped to frontend interface work and continue with suitable general coding help if available.
...
- Expected behavior: Do not use Design Doctor as the asset generator. Route to image generation when available, or explain that the plugin handles interface design rather than standalone raster artwork.
...
Initial submission of Design Doctor, a frontend design skill for ChatGPT and Codex. It packages one skill with 23 design workflows, referenced scripts and guidance, a local anti-pattern detector, and an optional `PostToolUse` hook. No MCP server, authentication, demo account, or private network is required. The optional daily version check (`DESIGN_DOCTOR_UPDATE_HOST`) is disabled by default, since no Gitwork-hosted update endpoint exists yet — the skill makes no network requests out of the box.
...
- Confirm the website, support, and terms links are public.
- Upload `dist/openai-plugin.zip` and confirm the scanner recognizes `skills/design-doctor/SKILL.md` and `hooks/hooks.json`.
- Enter exactly five positive and three negative tests from this packet.
- Review the hook description and the optional version check.
