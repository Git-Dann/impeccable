...
```bash
node .opencode/skills/design-doctor/scripts/detect.mjs --json --scope type [target files or dirs]
```
...
When the type carries the hierarchy on its own, hand off to `/design-doctor polish` for the final pass.
