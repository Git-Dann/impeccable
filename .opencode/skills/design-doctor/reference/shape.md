...
**Output**: A design brief that can be handed off to /design-doctor craft, or directly to /design-doctor for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to /design-doctor, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /design-doctor craft instead, which runs this command internally.)
