...
export function assembleLiveBrowserScript({ token, port, vocabulary, commandPrefix = '/', parts }) {
  const prelude =
    `window.__DESIGN_DOCTOR_TOKEN__ = '${token}';\n` +
    `window.__DESIGN_DOCTOR_PORT__ = ${port};\n` +
    `window.__DESIGN_DOCTOR_COMMAND_PREFIX__ = ${JSON.stringify(commandPrefix)};\n` +
    ...
    `window.__DESIGN_DOCTOR_VOCAB__ = ${JSON.stringify(vocabulary)};\n`;

  const body = parts.map((part) => {
    const file = part.file || path.basename(part.path || '');
    return `// --- design-doctor live script part: ${part.name} (${file}) ---\n${part.source}`;
  }).join('\n');
