...
export { DESIGN_DOCTOR_COMMAND_PREFIX } from './provider.mjs';

export const DESIGN_DOCTOR_DIR = '.design-doctor';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getDesignDoctorDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), DESIGN_DOCTOR_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd(), options = {}) {
  return path.join(getDesignDoctorDir(cwd, options), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd(), options = {}) {
  return path.join(getDesignDoctorDir(cwd, options), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env, targetPath } = {}) {
  if (env.DESIGN_DOCTOR_LIVE_CONFIG && env.DESIGN_DOCTOR_LIVE_CONFIG.trim()) {
    const configured = env.DESIGN_DOCTOR_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.design-doctor-live.json');
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.design-doctor-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd(), options = {}) {
  return path.join(getDesignDoctorDir(cwd, options), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.design-doctor-live', 'annotations');
}
