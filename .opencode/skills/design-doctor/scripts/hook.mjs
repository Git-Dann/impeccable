#!/usr/bin/env node
/**
 * Design Doctor design hook — PostToolUse entry point.
 *
...
async function main() {
  ...
  process.env.DESIGN_DOCTOR_HOOK_DEPTH = process.env.DESIGN_DOCTOR_HOOK_DEPTH || '1';
...
main().catch((err) => {
  ...
  if (process.env.DESIGN_DOCTOR_HOOK_DEBUG) {
    process.stderr.write(`[design-doctor-hook] ${err}\n`);
  }
  process.exit(0);
});
