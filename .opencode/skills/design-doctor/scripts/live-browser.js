/**
 * Design Doctor Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__DESIGN_DOCTOR_TOKEN__ and window.__DESIGN_DOCTOR_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__DESIGN_DOCTOR_LIVE_INIT__) return;
  window.__DESIGN_DOCTOR_LIVE_INIT__ = true;

  const TOKEN = window.__DESIGN_DOCTOR_TOKEN__;
  const PORT = window.__DESIGN_DOCTOR_PORT__;
  if (!TOKEN || !PORT) {
    window.__DESIGN_DOCTOR_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  // Brand kinpaku (gold) tokens for Accept / knobs / cycle-dots / the
  // selection outline / the comment tag, so they all match the Design
  // Doctor accent rather than a washed theme-adjusted one. Hardcoded here
  // (no external theme source) — keep in sync by hand if the brand
  // palette changes.
  const C = {
  ...
  // Picker bar chrome - quiet neutral elevation: no gold halo ring (gold is
  // reserved for the brand mark and the active control, not the container
  // outline).
  const PICKER_SHADOW =
  ...
  const PREFIX = 'design-doctor-live';
  const DESIGN_DOCTOR_COMMAND = (window.__DESIGN_DOCTOR_COMMAND_PREFIX__ || '/') + 'design-doctor';
  const PICK_CURSOR_STYLE_ID = PREFIX + '-pick-cursor-style';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__DESIGN_DOCTOR_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[design-doctor] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__DESIGN_DOCTOR_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__DESIGN_DOCTOR_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__DESIGN_DOCTOR_VOCAB__) ? window.__DESIGN_DOCTOR_VOCAB__ : [];
  const ICONS = {};
  ...
  let selectedAction = 'design-doctor';
  let selectedCount = 3;
  ...
  const domHelpers = window.__DESIGN_DOCTOR_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[design-doctor] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__DESIGN_DOCTOR_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__DESIGN_DOCTOR_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__DESIGN_DOCTOR_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-design-doctor-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-design-doctor-placeholder-resize]')?.dataset.designDoctorPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-design-doctor-editable], [data-design-doctor-original-text], [data-design-doctor-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-design-doctor-editable')
        || node.hasAttribute('data-design-doctor-original-text');
      node.removeAttribute('data-design-doctor-editable');
      node.removeAttribute('data-design-doctor-original-text');
      node.removeAttribute('data-design-doctor-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-design-doctor-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.designDoctorPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.designDoctorPlaceholderWidth;
    ...
    placeholder.dataset.designDoctorPlaceholderWidth = 'explicit';
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-design-doctor-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-design-doctor-variant]:not([data-design-doctor-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-design-doctor-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.designDoctorMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.designDoctorPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-design-doctor-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes design-doctor-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: design-doctor-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'design-doctor-spin 0.6s linear infinite',
      flexShrink: '0',
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[design-doctor] Refusing to render empty variant cycling state:', reason);
  ...
  //        data-design-doctor-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-design-doctor-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-design-doctor-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-design-doctor-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[design-doctor] Invalid data-design-doctor-params JSON:', err.message);
      return [];
    }
  }
  ...
  function applyParamValue(variantEl, param, value) {
    ...
    // [data-design-doctor-component-mount] with no [data-design-doctor-variant="N"]
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.designDoctorTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.designDoctorTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-design-doctor-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.designDoctorEditable = 'true';
      row.el.dataset.designDoctorOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.designDoctorEditable;
      delete row.el.dataset.designDoctorOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.designDoctorOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('design-doctor-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('design-doctor-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('design-doctor-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[design-doctor] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes design-doctor-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[design-doctor] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[design-doctor] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[design-doctor] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[design-doctor] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[design-doctor] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[design-doctor] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[design-doctor] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[design-doctor] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.designDoctorTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.designDoctorTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.designDoctorEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-design-doctor-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-design-doctor-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-design-doctor-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="design-doctor-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('design-doctor-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-design-doctor-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-design-doctor-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || '/src/lib/design-doctor/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-design-doctor-params`
  ...
  async function applySvelteComponentVariantStyle(variantNum) {
    ...
    if (!scopedCss) return;
    const style = document.createElement('style');
    style.dataset.designDoctorSvelteComponentStyle = sessionId;
    style.dataset.designDoctorVariant = String(variantNum);
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix = '[data-design-doctor-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[design-doctor] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[design-doctor] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.designDoctorVariants = sessionId;
      wrapper.dataset.designDoctorVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.designDoctorPreview = 'svelte-component';
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.designDoctorComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log('[design-doctor] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      console.error('[design-doctor] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[design-doctor] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- design-doctor-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- design-doctor-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[design-doctor] Variant wrapper not found in source file.');
          return;
        }
        ...
        const variants = wrapper.querySelectorAll('[data-design-doctor-variant]:not([data-design-doctor-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.designDoctorVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log('[design-doctor] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[design-doctor] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__designDoctorFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-design-doctor-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.designDoctorVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-design-doctor-variant="' + index + '"]');
  ...
  // single variant (`… > [data-design-doctor-variant="N"]`); without it, targets
  // every variant via the bare `[data-design-doctor-variant]` attribute.
  function variantStateSelector(sessionId, num) {
    const wrapper = '[data-design-doctor-variants="' + sessionId + '"]';
    const variant = num == null
      ? '[data-design-doctor-variant]'
      : '[data-design-doctor-variant="' + num + '"]';
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId)
      + ':not([data-design-doctor-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + ' }';
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-design-doctor-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-design-doctor-variants="' + sessionId + '"]') || n.querySelector?.('[data-design-doctor-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-design-doctor-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-design-doctor-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.designDoctorVariants || n.dataset?.designDoctorVariant) {
          ...
          if (n.querySelector?.('[data-design-doctor-variants],[data-design-doctor-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.designDoctorMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.designDoctorVariantCount || '0');
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.designDoctorMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-design-doctor-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${DESIGN_DOCTOR_COMMAND} init to generate one.`, 7000);
          console.log('[design-doctor] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[design-doctor] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[design-doctor] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[design-doctor] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[design-doctor] Failed to send event:', err);
        throw err;
      }
      console.debug('[design-doctor] Dropped optional live event:', err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.designDoctorOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__DESIGN_DOCTOR_LIVE_ADAPTER__ || '').toLowerCase();
    if (adapter === 'svelte' || adapter === 'sveltekit') return true;
    if (currentPreviewMode === 'svelte-component' || svelteComponentSession) return true;
    const wrapper = el?.closest?.('[data-design-doctor-variants]');
    return wrapper?.dataset?.designDoctorPreview === 'svelte-component';
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[design-doctor] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[design-doctor] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[design-doctor] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[design-doctor] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[design-doctor] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[design-doctor] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-design-doctor-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || acceptWrapper?.dataset?.designDoctorPreview === 'svelte-component';
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-design-doctor-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-design-doctor-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-design-doctor-variants],[data-design-doctor-variant],[data-design-doctor-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-design-doctor-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-design-doctor-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-design-doctor-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-design-doctor-css]');
    if (style && !document.querySelector('style[data-design-doctor-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-design-doctor-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-design-doctor-variants]');
    if (wrapper && wrapper.dataset.designDoctorPreview !== 'svelte-component') return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-design-doctor-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = 'none';
      setTimeout(function() {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-design-doctor-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-design-doctor-variant="original"]');
        if (orig) {
  ...
  // If a [data-design-doctor-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.designDoctorVariants;
    ...
    // wrapper holds a single mount target, not [data-design-doctor-variant] nodes,
    ...
    if (wrapper.dataset.designDoctorPreview === 'svelte-component'
    ...
    if (wrapper.dataset.designDoctorPreview === 'svelte-component') {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.designDoctorVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.designDoctorVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-design-doctor-variant]:not([data-design-doctor-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[design-doctor] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'design-doctor-live-pick';
  const INTERACTION_PREFS_KEY = 'design-doctor-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'design-doctor-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('design-doctor-dev-theme');
  ...
  function barPaletteForTheme(_theme) {
    // Picker chrome always uses neo-kinpaku styling, regardless of host page
    // light/dark theme.
    return {
      surface: C.ink,
      surfaceDeep: C.ink,
      ...
      // Verdigris patina - secondary state
  ...
  function attachSteerFocusGuard() {
    if (window.__DESIGN_DOCTOR_STEER_FOCUS_GUARD__) return;
    window.__DESIGN_DOCTOR_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('design-doctor-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[design-doctor.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__DESIGN_DOCTOR_STEER_FOCUS_DEBUG__) return;
    window.__DESIGN_DOCTOR_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'design-doctor-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[design-doctor.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[design-doctor.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes design-doctor-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes design-doctor-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes design-doctor-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: design-doctor-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: design-doctor-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Design Doctor mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'Design Doctor live mode'
      : 'Design Doctor live mode - agent not polling');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes design-doctor-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: design-doctor-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Design Doctor icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Design Doctor live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.design-doctor-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'design-doctor-ready'
  ...
  function loadDetectScript() {
    ...
    s.dataset.designDoctorExtension = 'true';
  ...
  function teardown() {
    ...
    removeVariantStateStylesheet();
    window.__DESIGN_DOCTOR_LIVE_INIT__ = false;
    console.log('[design-doctor] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .design-doctor/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'design-doctor-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .design-doctor/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Design Doctor-branded. The panel is
  // a viewer of the project's design system, not an Design Doctor surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>${DESIGN_DOCTOR_COMMAND} document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .design-doctor/design.json.</strong> Run <code>${DESIGN_DOCTOR_COMMAND} document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>${DESIGN_DOCTOR_COMMAND} document</code> also generates a <code>.design-doctor/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .design-doctor/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[design-doctor] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[design-doctor] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[design-doctor] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
