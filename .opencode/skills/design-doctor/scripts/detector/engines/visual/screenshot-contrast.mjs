...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `design-doctor-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('design-doctor-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'design-doctor-visual-contrast-hide-style';
      style.textContent = [
        '[data-design-doctor-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-design-doctor-visual-contrast-target][data-design-doctor-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-design-doctor-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-design-doctor-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-design-doctor-visual-contrast-target');
          el.removeAttribute('data-design-doctor-bgclip-text');
        }
      } catch {
