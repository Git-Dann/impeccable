 ...
 *
 * On first live run, the agent generates `.design-doctor/live/config.json`
...
import { resolveLiveConfigPath } from './lib/design-doctor-paths.mjs';
import {
...
const MARKER_OPEN_TEXT = 'design-doctor-live-start';
const MARKER_CLOSE_TEXT = 'design-doctor-live-end';
const IGNORE_MARKER_OPEN = '# design-doctor-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# design-doctor-live-ignore-end';
...
Reads configuration from .design-doctor/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .design-doctor/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*design-doctor-live-start\s*-->[\s\S]*?<!--\s*design-doctor-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*design-doctor-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*design-doctor-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
  for (const pat of patterns) {
    let changed = false;
...
// and stash the original `content` value in a `data-design-doctor-csp-original`
...
const CSP_MARKER_ATTR = 'data-design-doctor-csp-original';
