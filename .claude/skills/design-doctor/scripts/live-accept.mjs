 ...
 * <div data-design-doctor-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' design-doctor-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-design-doctor-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' design-doctor-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' design-doctor-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-design-doctor-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-design-doctor-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-design-doctor-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-design-doctor-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-design-doctor-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-design-doctor-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-design-doctor-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-design-doctor-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'design-doctor-variants-start ' + id;
  const endPattern = 'design-doctor-variants-end ' + id;
 ...
 * the `<div data-design-doctor-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-design-doctor-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-design-doctor-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('design-doctor-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-design-doctor-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-design-doctor-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-design-doctor-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-design-doctor-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-design-doctor-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-design-doctor-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = 'design-doctor-variants-start ' + id;
