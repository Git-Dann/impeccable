 ...
 *   4. $DESIGN_DOCTOR_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx design-doctor update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

// No Gitwork-hosted update-check endpoint exists yet, so this is disabled
// unless a deployment explicitly points it at one via the env var.
const UPDATE_HOST = (process.env.DESIGN_DOCTOR_UPDATE_HOST || '').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.DESIGN_DOCTOR_UPDATE_CACHE || path.join(os.homedir(), '.design-doctor', 'update-check.json');
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.DESIGN_DOCTOR_CONTEXT_DIR;
...
// outside both the project and repo roots (DESIGN_DOCTOR_CONTEXT_DIR override).
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Design Doctor skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Design Doctor (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx design-doctor update\`." ` +
    `If they agree, run \`npx design-doctor update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with DESIGN_DOCTOR_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.design-doctor', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.DESIGN_DOCTOR_NO_UPDATE_CHECK) return null;
...
async function cli() {
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = [
      ...
      `offer \`${DESIGN_DOCTOR_COMMAND} init\` as a suggestion (do not block).`,
    ];
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    'Ask the user which app Design Doctor should use, then rerun Design Doctor helper commands from that child app cwd using this same scripts directory. ' +
