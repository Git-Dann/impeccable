...
import {
  ...
  DESIGN_DOCTOR_COMMAND_PREFIX,
  readLiveServerInfo,
  removeLiveServerInfo,
  resolveDesignSidecarPath,
  writeLiveServerInfo,
} from './lib/design-doctor-paths.mjs';
...
// DESIGN sidecar is project-local at .design-doctor/design.json, with legacy
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.DESIGN_DOCTOR_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'design-doctor', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
  ...
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      const body = assembleLiveBrowserScript({
        ...
        commandPrefix: DESIGN_DOCTOR_COMMAND_PREFIX,
        parts,
      });
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .design-doctor/design.json
    ...
    //                          - sidecar: .design-doctor/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .design-doctor/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!normalized.includes('node_modules/.design-doctor-live/') && !normalized.includes('src/lib/design-doctor/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[design-doctor] Svelte component session cleanup failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[design-doctor] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[design-doctor] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nDesign Doctor live server running on ${url}`);
