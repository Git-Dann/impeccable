 ...
 *
 * Variants are real .svelte components under node_modules/.design-doctor-live/<session-id>/.
...
export const SVELTE_COMPONENT_ROOT = 'node_modules/.design-doctor-live';
export const SVELTE_RUNTIME_FILE = `${SVELTE_COMPONENT_ROOT}/__runtime.js`;
export const DEFERRED_ACCEPTS_FILE = '.design-doctor/live/deferred-svelte-component-accepts.json';

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.DESIGN_DOCTOR_LIVE_SVELTE_COMPONENT || '')) return false;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="design-doctor-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .design-doctor-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = 'div') {
  ...
  if (!/data-design-doctor-variant|design-doctor-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--design-doctor-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-design-doctor-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--design-doctor-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-design-doctor-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, 'g');
}
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-design-doctor-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: 'Accepted Svelte insert variant contains preview-only data-design-doctor attributes', ...resultBase };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), 'design-doctor-live', key, 'deferred-svelte-component-accepts.json');
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-design-doctor-params attribute.',
      'Do not use @scope or data-design-doctor-variant selectors in component files.',
    ...
    forbidden: [
      ...
      'Do not add data-design-doctor-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-design-doctor-params with JSON breaks the build; use componentDir/params.json instead.',
    ],
