 ...
 *
 * `pin audit` creates a lightweight audit skill that redirects to Design Doctor's audit workflow.
...
const PIN_MARKER = '<!-- design-doctor-pinned-skill -->';
...
/**
 * Find harness skill directories that have an design-doctor skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have design-doctor installed
    const designDoctorDir = join(skillsDir, 'design-doctor');
    if (existsSync(designDoctorDir) || existsSync(join(skillsDir, 'i-design-doctor'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix) {
  const desc = metadata[command]?.description || `Shortcut for ${commandPrefix}design-doctor ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}design-doctor ${command}\`.

Invoke ${commandPrefix}design-doctor ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with design-doctor installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use Design Doctor's '${command}' workflow directly to access it.`);
  } else {
