 ...
 * an ignore in `.design-doctor/config.json`.
 *
 ...
 * line, so the same marker works across every comment style design-doctor scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   design-doctor-disable <rule>[, <rule>...] [-- reason]   whole file
 *   design-doctor-disable-line <rule>...      [-- reason]   the same line
 *   design-doctor-disable-next-line <rule>... [-- reason]   the following line
 *   design-doctor-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- design-doctor-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* design-doctor-disable-line overused-font *\/
 *   // design-doctor-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /design-doctor-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `Design-Doctor-Disable`).
  if (!/design-doctor-disable/i.test(text)) return result;
