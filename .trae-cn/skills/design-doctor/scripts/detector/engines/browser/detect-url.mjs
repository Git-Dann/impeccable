...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.designDoctorAnalyzeVisualContrast !== 'function') return [];
        return window.designDoctorAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.designDoctorCollectVisualContrastCandidates !== 'function') return [];
      return window.designDoctorCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
...
async function detectUrl(url, options = {}) {
  ...
  try {
    ...
    await profileStepAsync(profile, {
      ...
      window.__DESIGN_DOCTOR_CONFIG__ = {
        ...(window.__DESIGN_DOCTOR_CONFIG__ || {}),
        autoScan: false,
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.designDoctorDetect) return [];
        return window.designDoctorDetect({ decorate: false, serialize: true });
      });
