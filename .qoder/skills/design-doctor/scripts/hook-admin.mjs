#!/usr/bin/env node
/**
 * The Design Doctor hooks command manages the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .design-doctor/config.json / .design-doctor/config.local.json.
 *
 * Usage:
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/design-doctor',
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/design-doctor/scripts/hook.mjs"',
  ...
  {
    provider: '.agents',
    skillRel: '.agents/skills/design-doctor',
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/design-doctor/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
  ...
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/design-doctor',
  ...
  },
  {
    ...
    provider: '.github',
    skillRel: '.github/skills/design-doctor',
    destRel: '.github/hooks/design-doctor.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/design-doctor/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive an Design Doctor hooks edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.DESIGN_DOCTOR_HOOK_DISABLED;
  const envState = envKill ? `DESIGN_DOCTOR_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.design-doctor/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.design-doctor/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.design-doctor/hook.cache.json';
...
function fileHasDesignDoctorHookMarker(filePath) {
...
function valueHasDesignDoctorHookMarker(value) {
  ...
  if (value && typeof value === 'object') return Object.values(value).some(valueHasDesignDoctorHookMarker);
  return false;
}
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error(`Pass a rule id, e.g. ${DESIGN_DOCTOR_COMMAND} hooks ignore-rule side-tab`);
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error(`overused-font is value-specific by default. Use ${DESIGN_DOCTOR_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${DESIGN_DOCTOR_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`);
  }
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error(`Pass a glob, e.g. ${DESIGN_DOCTOR_COMMAND} hooks ignore-file "src/legacy/**"`);
  const config = mergeDetectorConfig(readRawDetectorConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(`Pass a rule id and value, e.g. ${DESIGN_DOCTOR_COMMAND} hooks ignore-value overused-font Inter`);
  }
