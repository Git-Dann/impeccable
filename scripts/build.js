...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /design-doctor. Count them from the command router table in SKILL.md.
  const rootSkill = skills.find(s => s.name === 'design-doctor');
  let commandCount;
  if (rootSkill) {
    ...
    const routerMatches = rootSkill.body.match(/^\| `[^`]+` \|/gm);
  ...
  // No marketing site to write generated counts into — this build only ships
  // the skill/CLI/extension. Keep the validation pass below (README/AGENTS/
  // plugin manifests) so stale count references still get caught.
...
/**
 * Narrow prose check for the design-doctor skill source.
 *
...
`Design Doctor. Design fluency for AI harnesses.
https://github.com/git-dann/impeccable
...
async function build() {
  ...
  // No marketing site in this fork — this script focuses purely on the
  // skill/CLI/extension provider outputs.
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    const deprecatedLocalSkills = [
      'frontend-design', 'teach-design-doctor',
      'arrange', 'normalize', 'onboard', 'extract',
      // v3.0 consolidation: standalone skills -> /design-doctor sub-commands
    ...
    // dirs above stay where they are because `npx skills add git-dann/impeccable`
    ...
    const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'design-doctor');
    if (fs.existsSync(claudeSkillsSrc)) {
      fs.mkdirSync(pluginSkillsDir, { recursive: true });
      copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'design-doctor'));
    }
