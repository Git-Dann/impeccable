...
export const SUITES = {
  core: {
    ...
    triggers: [
      ...
      /^skill\/(SKILL\.src\.md|agents\/|reference\/|scripts\/(cleanup-deprecated|context|context-signals|critique-storage|design-parser|hook|design-doctor-paths|is-generated|lib\/provider|pin))/,
      /^README(\.npm)?\.md$/,
      /^cli\/bin\//,
      /^tests\/(build|cleanup-deprecated|cli-ignores|context|context-signals|critique-storage|design-parser|github-sheriff|hook|hook-build|design-doctor-paths|openai-plugin|pin|skills-cli|target-args|test-suites|windows-path-fix|zip)\.test\.(js|mjs)$/,
      /^tests\/lib\//,
    ],
    commands: [
      {
        ...
        files: [
          ...
          'tests/lib/design-doctor-config.test.js',
      ...
      },
      {
        ...
        files: [
          ...
          'tests/hook.test.mjs',
          'tests/design-doctor-paths.test.mjs',
  ...
  'cli-remote-e2e': {
    description: 'Remote CLI install/update smoke tests against the git-dann/impeccable GitHub release.',
    optIn: true,
    triggers: [
      ...COMMON_INFRA_PATTERNS,
    ...
    commands: [
      {
        runner: 'bun',
        env: { DESIGN_DOCTOR_CLI_REMOTE_E2E: '1' },
        files: ['tests/skills-cli.test.js'],
      },
    ],
  },
